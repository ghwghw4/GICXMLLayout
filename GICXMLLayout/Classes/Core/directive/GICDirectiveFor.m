...
-(void)updateDataSource:(id)dataSource{
    ...
    if([dataSource isKindOfClass:[NSArray class]] && [self.target respondsToSelector:@selector(gic_addSubElement:)]){
        ...
        if([dataSource isKindOfClass:[NSMutableArray class]]){
            ...
            // 插入
            [[dataSource rac_signalForSelector:@selector(insertObjects:atIndexes:)] subscribeNext:^(RACTuple * _Nullable x) {
                @strongify(self)
                for(id data in x[0]){
                    [self insertAElement:data index:[dataSource indexOfObject:data]];
                }
            }];
